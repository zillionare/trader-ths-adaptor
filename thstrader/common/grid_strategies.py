...
if TYPE_CHECKING:
    ...
    from thstrader import clienttrader
...
class Xls(BaseStrategy):
    ...
    def has_captcha(self):
        if (
            not self._trader.app.top_window()
            .window(
                class_name="Static", control_id=self._trader._config.POSITION_CAPTCHA
            )
            .exists(timeout=1)
        ):
            logger.info("未找到验证码窗口")
            return False
        return True
    ...
    def handle_captcha(self):
        ...
        while count > 0:
            try:
                self._trader.app.top_window().window(
                    control_id=self._trader._config.POSITION_CAPTCHA,
                    class_name="Static",
                ).capture_as_image().save(
                    file_path
                )  # 保存验证码
            ...
            captcha_num = captcha_recognize(
                file_path, config="-c tessedit_char_whitelist=0123456789"
            ).strip()  # 识别验证码
            captcha_num = "".join(captcha_num.split())
            logger.info("captcha result-->" + captcha_num)
            if len(captcha_num) == 4:
                if not self._trader._editor_need_type_keys:
                    self._trader.app.top_window().window(
                        control_id=self._trader._config.POSITION_CAPTCHA_EDIT,
                        class_name="Edit",
                    ).set_edit_text(
                        captcha_num
                    )  # 模拟输入验证码
